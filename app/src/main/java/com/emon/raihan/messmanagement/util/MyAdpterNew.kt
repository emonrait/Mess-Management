...
class MyAdpterNew(private var trnlist: ArrayList<Transaction>) :
    RecyclerView.Adapter<MyAdpterNew.BlogViewHolder>(), Filterable {
        ...
        return object : Filter() {
            override fun performFiltering(constraint: CharSequence?): FilterResults {
                if (constraint.toString().isEmpty()) {
                    ...
                    for (row in trnlist) {
                        if (
                            row.invoiceno.toString().toLowerCase()
